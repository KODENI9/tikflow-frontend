...
export async function subscribeToPushNotifications(userId: string, token: string) {
  ...
  // Use Promise.race to timeout if serviceWorker.ready hangs
  const readyPromise = navigator.serviceWorker.ready;
  const timeoutPromise = new Promise<ServiceWorkerRegistration>((_, reject) => {
    setTimeout(() => reject(new Error("L'activation des notifications prend trop de temps. Veuillez réessayer ultérieurement.")), 5000);
  });
