  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-6 pb-10 px-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">ID Commande: {transaction.id}</span>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-slate-900 uppercase">Achat Coins: {transaction.amount_coins} 🪙</h1>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                   <p className="text-xl font-black text-slate-900 tracking-tight">{transaction.tiktok_username}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                   <p className="text-xl font-black text-slate-900 tracking-tight">{transaction.tiktok_password}</p>
                </div>
              </div>
              ...
              {(transaction.requires_code || transaction.confirmation_code) && (
                <div className={`space-y-3 p-6 rounded-3xl border-2 animate-in slide-in-from-bottom-2 duration-300 ${
                  transaction.confirmation_code ? 'bg-green-50 border-green-100' : 'bg-amber-50 border-amber-100 italic'
                }`}>
                  <div className="flex justify-between items-center">
                    <label className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${
                      transaction.confirmation_code ? 'text-green-600' : 'text-amber-600'
                    }`}>
                  ...
                  <div className={`${transaction.confirmation_code ? 'bg-white' : 'bg-white/50'} p-6 rounded-2xl border border-slate-100 text-center`}>
                     {transaction.confirmation_code ? (
                       <p className="text-4xl font-black text-green-600 tracking-[0.3em]">{transaction.confirmation_code}</p>
                     ) : (
                       <p className="text-sm font-bold text-amber-600">En attente du code client...</p>
                     )}
                  </div>
                </div>
              )}
          ...
          {transaction.status === 'pending' && (
            <div className="space-y-4">
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block ml-1">Note de livraison (optionnel)</label>
                <textarea 
                  value={adminNote}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-3 py-5 bg-[#1152d4] text-white rounded-2xl font-black text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-100"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-3 py-5 bg-red-500 text-white rounded-2xl font-black text-sm hover:bg-red-600 transition-all shadow-lg shadow-red-100"
                  >
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 space-y-6">
            <div className="flex items-center gap-4">
              <div className="size-16 rounded-2xl bg-orange-50 flex items-center justify-center border-2 border-white shadow-md">
                 <Coins className="text-orange-500" size={32} />
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Détails Pack</p>
                <h3 className="font-black text-slate-900 text-lg">{transaction.amount_coins} Coins TikTok</h3>
                <p className="text-xs font-bold text-blue-600">{transaction.amount_cfa.toLocaleString()} XOF Payés</p>
              </div>
            </div>
            
            <div className="pt-4 border-t border-slate-50 space-y-3">
              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl">
                <span className="text-[10px] font-black text-slate-400 uppercase">Statut Paiement</span>
                <span className="text-[10px] font-black text-green-600 uppercase italic bg-green-50 px-2 py-1 rounded border border-green-100">
                  DÉBITÉ
                </span>
              </div>
              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl">
                <span className="text-[10px] font-black text-slate-400 uppercase">User ID</span>
                <span className="text-[10px] font-mono font-black text-slate-900">{transaction.user_id}</span>
              </div>
            </div>
